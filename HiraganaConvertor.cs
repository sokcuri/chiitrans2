...
namespace ChiiTrans
{
    ...
    {
        private static HiraganaConvertor _instance;
        public static HiraganaConvertor instance
        {
            get
            {
                if (_instance == null)
                    _instance = new HiraganaConvertor();
                return _instance;
            }
        }
        ...
        public string Convert(string src)
        {
            ...
            for (int i = 0; i < src.Length; ++i)
            {
                if (i + 1 < src.Length)
                {
                    ...
                    if (b.dict.ContainsKey(tmp))
                    {
                        res.Append(b.dict[tmp]);
                        i += 1;
                        continue;
                    }
                }
                string ch = "" + src[i];
                if (a.dict.ContainsKey(ch))
                {
                    res.Append(a.dict[ch]);
                }
                else
                {
                    res.Append(ch);
            ...
            result = Regex.Replace(result, "っ ([A-Za-z])", match => match.Groups[1].Value + match.Groups[1].Value);
            result = Regex.Replace(result, "っ([A-Za-z])", match => match.Groups[1].Value + match.Groups[1].Value);
            result = result.Replace("っ", "");
            result = result.Replace("u゜", "ū").Replace("o゜", "ō").Replace("i゜", "ī").Replace("a゜", "ā").Replace("e゜", "ē");
            result = Regex.Replace(result, @"(?<=[\?\!\.「…‥・“]\s*)(\w)", match => match.Groups[1].Value.ToUpper());
